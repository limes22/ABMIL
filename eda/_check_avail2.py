#!/usr/bin/env python3
"""Check ECC-DI + Vanilla availability across 9 (ds, fe) × 3 heads."""
import os, re, glob
...
def has_finished_split(folder_pattern):
    """folder_pattern like {prefix}_s*_*. Returns set of seeds with at least split_0_results.pkl."""
    seeds = set()
    for d in glob.glob(f"{RESULTS}/{folder_pattern}"):
        m = re.search(r"_s(\d+)_\d{8}_\d{6}_s\d+$", d)
        if not m: continue
        if os.path.exists(f"{d}/split_0_results.pkl"):
            seeds.add(int(m.group(1)))
    return sorted(seeds)

print(f"{'ds':<6} {'fe':<8} {'head':<6} {'feat':<25} {'ECCDI5_seeds':<18} {'Vanilla_seeds':<18}")
print("-" * 100)
for ds in DATASETS:
    for fe in BACKBONES:
